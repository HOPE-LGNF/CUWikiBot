...
- 巡查与 Shortpages 独立降级：缺权、身份未知或登录失败时跳过巡查；认证身份失效后不再发送巡查请求。身份警告仅使身份未知，除下述已知登录提示外的 API 警告保守处理，不能把忽略了过滤条件的响应当作有效计数。
...
### 登录提示与巡查字段修复（2026-09-09）

`_is_login_advisory()` 仅在 `action=login` 中精确匹配主账号登录接口弃用提示及 API 公告订阅提示。未知模块、文本变化或混入其他警告仍失败；提示被接受后，仍由 `_login()` 检查实际登录结果及规范用户名，保留有限 clientlogin 回退与后续身份断言。

巡查使用 `rcprop=ids|flags|title`：`title` 同时返回命名空间 `ns`，供记录与范围校验使用。仅有 `ids|flags` 时会缺少该字段。增加返回字段不改变筛选条件、首批上限或计数口径。

功能降级日志增加 `stage=login/shortpages/patrol`，仍只输出安全分类、受限错误码和 HTTP 状态，不输出原始警告、响应或凭据。`WikiStatsPlugin.handle()` 的签名、Markdown Reply、刷新按钮及无用户 ID 时省略按钮的行为保留。
...
2026-09-09 本次修复验证（WSL Ubuntu，项目原有 Python 3.10 虚拟环境）：15 项标准库回归测试通过，覆盖登录提示下巡查及身份断言、其他警告和真实登录失败、携带提示的有限登录回退，以及巡查字段依赖。原有框架 Reply 和按钮测试保留。Ruff 格式、lint（preview）和 ty 按原项目业务模块范围通过。本次未启动完整机器人，未使用目标项目凭据进行实站登录或真实 QQ 发送；NoneBot 版本的实站成功不替代本项目验收。

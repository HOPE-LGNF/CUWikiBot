...
巡查展示“待巡查更改，及其中新建”，统计同一批未巡查的编辑／新建记录。只涵盖当前身份可见的 RecentChanges 保留窗口，不是历史累计。顶部 `PATROL_LIMIT` 默认 500（允许 1–500），`PATROL_NAMESPACES` 默认 `None` 表示所有可见命名空间，设为 `(0,)` 可只查主空间。始终只读一批，有后续时两项都仅代表已读部分。无权限、身份未知、影响查询语义的 API 警告或访问失败不会伪装成 0。
...
2026-09-09 修复了登录弃用提示被误判为失败、巡查请求缺少命名空间字段两处问题。现在只放行已确认的登录提示，继续校验登录结果与身份，并为巡查请求补齐 `rcprop=title` 返回的 `ns`。更新后重启机器人即可。

降级日志通过 `stage=login/shortpages/patrol` 标明阶段，再结合 `kind`、`code`、`status` 判断原因；HTTP 200 不代表业务成功，`status=None` 也可能表示前置检查跳过请求。实现边界和验证记录见 [PLUGIN_ARCHITECTURE.md](PLUGIN_ARCHITECTURE.md)。
